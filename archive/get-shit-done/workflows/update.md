...
(Paths are relative to your install location: `/Users/nick/.claude/` for global, `./.claude/` for local)
...
rm -f /Users/nick/.claude/cache/gsd-update-check.json
```

The SessionStart hook (`gsd-check-update.js`) always writes to `/Users/nick/.claude/cache/` via `os.homedir()` regardless of install type, so both paths must be cleared to prevent stale update indicators.
</step>
